...
def fasta_to_pdb_csv(fasta_file, output_csv, identity_cutoff=0.2, rows=1000, delay=1):
    ...
    # CSV will contain ONLY PDB-level info
    fieldnames = [
        "pdb_id", "Resolution", "pubmed_id", "apparatus", "atmosphere",
        "crystal_id", "details", "method", "method_ref", "pH",
        "pressure", "pressure_esd", "seeding", "seeding_ref",
        "temp", "temp_esd", "temp_details", "time",
        "pdbx_details", "pdbx_pH_range"
    ]

    seen_pdb_ids = set()  # <-- prevents repetition
        ...
        for query_id, sequence in sequences.items():
            ...
            for entry in data.get("result_set", []):
                ...
                try:
                    ...
                    print(f"[WARN] {pdb_id}: crystallization extraction failed: {e}")
                    continue

                # Build row with ONLY allowed fields
                row = {"pdb_id": pdb_id}
                row.update(crystallization)
                row = {k: row.get(k, "") for k in fieldnames}

                writer.writerow(row)
                f.flush()  # allow live reading

            #time.sleep(delay)

    print(f"PDB crystallization table saved to {output_csv}")
